...
``workload-name``     Kubernetes pod's workload (Deployment, Statefulset, Daemonset, ReplicationController, CronJob, Job, DeploymentConfig (OpenShift)).
...
These metrics should not be enabled at the same time as ``httpV2``.
...
These metrics should not be enabled at the same time as ``http``.
