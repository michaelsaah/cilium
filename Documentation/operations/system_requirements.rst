...
``systemd`` 245 and above overrides the ``rp_filter`` setting of Cilium network
interfaces. This introduces connectivity issues (see :gh-issue:`10645` for
details). To avoid that, configure ``rp_filter`` in ``systemd`` using a command
...
Keep in mind, however, that some distributions may require additional
overrides. For example, in Ubuntu 22.04 it's also necessary to edit
``/etc/sysctl.d/10-network-security.conf``, otherwise this change will get
undone over time. Please refer to your distribution's documentation for
the right way to perform this override.

Some distributions additionally need to be configured not to manage "foreign"
routes. This is the case in Ubuntu 22.04, for example (see :gh-issue:`18706`).
This can usually be done by setting
